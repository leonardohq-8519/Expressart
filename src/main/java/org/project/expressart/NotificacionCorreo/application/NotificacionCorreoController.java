...
public class NotificacionCorreoController {
    ...
    public ResponseEntity<List<NotificacionCorreoResponseDTO>> getByUser(@PathVariable Long userId) {
    ...
    @PostMapping
    public ResponseEntity<NotificacionCorreoResponseDTO> create(@RequestBody NotificacionCorreoCreateDTO dto) {
        return ResponseEntity.status(HttpStatus.CREATED).body(emailNotificationService.create(dto));
    }

    @PatchMapping("/{id}/status")
    public ResponseEntity<Void> updateStatus(@PathVariable Long id, @RequestBody NotificacionCorreoEstadoDTO dto) {
